    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* Title with gradient - FIXED: removed break-words */}
          <h2 className={`brand-title text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold mb-4 sm:mb-6 ${myFont.className}`}>
            <span className="bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 bg-clip-text text-transparent animate-gradient inline-block">
              {title}
            </span>
          </h2>

          {/* Subtitle - FIXED: proper text wrapping */}
          <p className="max-w-2xl mx-auto text-sm sm:text-base lg:text-lg text-gray-300 leading-relaxed px-4 text-balance">
            {subtitle}
          </p>
        </motion.div>
        ...
        {showServiceFilter && (
          ...
          >
            <div className="flex items-center justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <Filter className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors flex-shrink-0 ${isFilterActive ? 'text-purple-400' : 'text-gray-400'}`} />
              ...
              >
                ...
                {remainingCount > 0 && (
                  <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <span className="text-xs sm:text-sm text-gray-400 text-center px-2 whitespace-nowrap">
                      +{remainingCount} more {remainingCount === 1 ? 'project' : 'projects'}
                    </span>
                ...
                >
                  ...
                  <span className={`relative text-xs sm:text-sm tracking-wider whitespace-nowrap ${myFont.className}`}>
                    View All Projects
                  </span>
                  <ArrowRight size={16} className="transition-transform group-hover:translate-x-1 sm:w-[18px] sm:h-[18px] flex-shrink-0" />
                </motion.button>
